...
static int print_schedule(struct bpf_map *proc_map, struct bpf_map *sys_map, int schedmap_fd)
{
	int err,key = 0;
	...
	struct proc_sched_event proc_event;
	struct sys_schedule sys_event;
	time_t now = time(NULL);
	struct tm *localTime = localtime(&now);
    int hour = localTime->tm_hour;
    int min = localTime->tm_min;
	...
	while (!bpf_map_get_next_key(proc_fd, &lookup_key, &next_key)) {
		...
		proc_avg_delay = proc_event.sum_delay/(proc_event.count == 0 ? 1 : proc_event.count);
...
int main(int argc, char **argv)
{
	...
	while (!exiting) {
		...
		if(env.enable_schedule && env.output_schedule){
			err = print_schedule(schedule_skel->maps.proc_schedule, schedule_skel->maps.sys_schedule, schedmap_fd);
			/* Ctrl-C will cause -EINTR */
			if (err == -EINTR) {
				err = 0;
				break;
