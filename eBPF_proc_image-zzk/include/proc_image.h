...
struct syscall_val_t {
	int syscall_id;
	u64 timestamp;       // enter: enter time               exit: exit time
	u64 duration;
	int ret;
}
// struct syscall_seq{
...
struct sched_ctrl {
    bool sched_func;
    pid_t target_pid;   // target pid > 0
    // int target_cpu_id;
    // int target_tgid;
};

struct proc_sched_event{
	int pid;
	int prio;
	int count;
	long long unsigned int enter_time;
...
struct target_sched_event{
	int cpu_id;
	long long unsigned int delay;
	long long unsigned int ktime;
};
struct sys_schedule{
